...
změna pozadí záhlaví
@navbar_background_image: linear-gradient(12deg, blue, blue 15%, #ffcc00);
změna primárních barev (používaných pro tlačítka)
@primaryColor: 'blue';

b. změna barvy pozadí zápatí
...
Příkazová řádka `nrp` umožňuje vytvářet, číst, upravovat a mazat záznamy a soubory. Nejprve zaregistrujeme repozitář:
...
```bash
nrp create record '{"title": "My first dataset"}' --set myrec

nrp read record @myrec

nrp update record @myrec '{"title": "My first dataset - updated"}'

nrp read record @myrec

nrp upload file @myrec invenio.cfg

nrp list files @myrec
```
...
invenio index init
invenio oarepo cf init
invenio oarepo communities custom-fields init
...
Příkaz vytvoří soubor v `ui/datasets/templates`.

### Kontrola

Spusťte server a otevřete stránku s detailem záznamu
