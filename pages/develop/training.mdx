...
```bash
invenio users create -a -c "manager@e-infra.cz" --profile '{"full_name": "Velky Manazer"}'
```
...
A následně vytvořte uživatele `member@e-infra.cz` a nastavte mu roli "member" ve stejné komunitě.
