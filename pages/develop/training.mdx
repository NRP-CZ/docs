...
```bash
invenio users create -a -c "ja@me.com" --profile '{"full_name": "Ja I"}'
```

```bash
invenio oarepo communities members add generic "ja@me.com" owner
```

A následně vytvořte dalšího uživatele a nastavte mu roli "member" ve stejné komunitě.

### Kontrola

Přihlaste se do repozitáře jako vytvořený uživatel a ujistěte se, že vidíte komunitu `Generic` ve svém dashboardu.
