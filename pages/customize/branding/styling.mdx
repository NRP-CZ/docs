...
## Global Stylesheet
The `branding.less` file is used for global styling rules, such as any font faces or CSS reset styles, that should be applied once and before any other component-specific styles:
...
The `site.overrides` file allows you to define or override default LESS style rules that gets inherited by every existing UI component:
...
Be cautious when using this file, as any rules added here will included into page as a base stylesheet for each and every existing UI component,
increasing the page load time.
People commonly confuse this file with the [global stylesheet](#global-stylesheet), which gets included into page only once. Using the global
stylesheet is the preferred way to apply any page or site-wide styling.
</Callout>
