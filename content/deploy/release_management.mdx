...
- other `oarepo-*` packages they use
- Invenio packages referenced in source imports or tests
...
Use [resolve_invenio.sh](https://github.com/oarepo/oarepo/blob/main/tools/resolve_invenio.sh)
to get a list of Invenio packages and recommended version ranges for dependency declarations.
Whenever you add a new import, run this tool to check whether a new dependency is required.
...
1. Create a PR with the change. Inside the PR, set package version to `X.0.0` (where X is incremented)
2. **TODO:** Open [oarepo-app actions](https://github.com/oarepo/oarepo-app/actions) and run the dependency checker.
3. Provide:
   - target `oarepo` version (empty means the latest version)
   - list of `oarepo-*` packages (the checker will suppose that major version of these packages will be changed,
     that is the PR of the change IS NOT MERGED yet)
4. Use the generated dependency graph to identify all packages that require a major bump.
5. Create PRs for all affected packages:
   - set package version to `X.0.0`
   - update dependency constraints in `pyproject.toml` to new major ranges where needed
6. Run the testrig with all related PRs:
...
### 6) Update the app copier template

When making a major release of `oarepo-app`, update the
[`pyproject.toml` template in the app copier](https://github.com/oarepo/nrp-app-copier/blob/rdm-14/template/pyproject.toml.copier)
to contain the new `oarepo-app` version range.
