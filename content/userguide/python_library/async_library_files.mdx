...
# Download multiple files in parallel with connection limit
async def download_file(file, output_dir):
    output_path = f"{output_dir}/{file.key}"
    await client.files.download(file, sink=output_path)
    return file.key

async def download_all_parallel(record, output_dir):
    files = await client.files.list(record)
    
    # Limit to 5 concurrent downloads
    async with limit_connections(5):
        tasks = [download_file(file, output_dir) for file in files]
        results = await asyncio.gather(*tasks)
    
    return results

# Use it
downloaded = await download_all_parallel(record, "/downloads")
print(f"Downloaded {len(downloaded)} files")
```

### Download with Multipart (Parallel Parts)

```python
# Download large file in parallel parts
await client.files.download(
    file,
    sink="/path/to/large-file.zip",
    parts=4,  # Download in 4 parallel parts
    progress="Downloading in parallel"
)
```
...
async def copy_files_between_records(source_record_id, dest_record_id):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for file in source_files:
            ...
            await client.files.download(file, sink=str(tmp_path))
