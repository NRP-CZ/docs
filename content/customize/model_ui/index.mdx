...
1. **Auto-generated components (default)** — The system automatically generates a JinjaX component at runtime that extends the appropriate base template. No configuration required.
...
<Callout type="info">
**Backward compatibility:** Existing configurations using `templates = {...}` continue to work unchanged. The auto-generation feature only applies when no template is explicitly configured for a given page type.
</Callout>

## Runtime Page Component Generation

When a page template is not explicitly configured, the system automatically generates a JinjaX component at runtime. This feature eliminates the need for boilerplate configuration while still allowing full customization when needed.
...
- Is cached for performance on subsequent requests

### Page Types and Base Templates

Each page type has a default base template that the generated component extends:
...
- **Use auto-generated components** when the default layout and behavior meet your needs
