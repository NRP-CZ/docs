...
| `saveOnTabChange` | bool   | Forces a save when leaving this tab, even if the form is not dirty. Useful for sections like file upload or community selection that update Redux state rather than Formik state, so the form won't appear dirty despite having unsaved changes. |
