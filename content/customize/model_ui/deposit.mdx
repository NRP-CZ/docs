...
Create a sections file that defines each tab in the wizard:
...
| `render`        | function | A React component that will be rendered into the tab panel for this section. Available props: `{ record, formConfig, activeStep, next, back, initialRecord }` |
...
If you're using a shared library that provides predefined sections (like `ccmm-invenio`), you can import them and extend with your own:
...
export const mySections = [
  ...
  {
    key: "custom-metadata",
    label: i18next.t("Custom metadata"),
    render: () => (
      <>
        <TextField fieldPath="metadata.custom_field_1" />
        <TextField fieldPath="metadata.custom_field_2" />
      </>
    ),
    includesPaths: ["metadata.custom_field_1", "metadata.custom_field_2"],
  },
];
```
...
const componentOverrides = {
  [`${overridableIdPrefix}.FormFields.container`]: FormFieldsContainer,
  // Add content below the "basic-information" section
  "OarepoUI.TabContent.basic-information": BasicInfoExtra,
};
```
