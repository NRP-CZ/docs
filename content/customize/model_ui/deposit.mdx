...
- **Wizard of sections** — the form renders as a wizard with vertical tabs, built from an **ordered array of section objects** (one tab each). You customize the form by composing and extending this array, not by editing a monolithic form file.
...
Create a section file per logical group of fields. A section is a plain object — `key` (used in the `?tab=key` URL), `label`, `component` returning your fields, and `includesPaths` (used for error indicators and the per-tab completion bar):
...
export const BasicInformationSection = {
  ...
  component: ({ record, formConfig }) => (
    <React.Fragment>
      ...
      <CreatibutorsField fieldPath="metadata.creators" schema="creators" />
    </React.Fragment>
  ),
  includesPaths: [
    "metadata.title",
...
Field labels, hints and help texts come from the model schema automatically (`useFieldData`) — you only place the components. Collect your sections in one file, composing them with the preset sections shipped with your base model:
...
<Callout type="info">
  ...
  components](#create-custom-form-components).
</Callout>
...
export const BasicInformationSection = {
  ...
  includesPaths: ["metadata.title", "metadata.description", "metadata.creators"],
...
Import and use your custom fields inside your section components:
...
```jinja filename="ui/mymodel/templates/semantic-ui/mymodel/DepositCreate.jinja"
...
<Callout type="info">
  The base template automatically includes this file if it exists at `
  {model_name}/deposit_create/javascript.html`.
</Callout>
...
class ExtraContextComponent(UIResourceComponent):
    def before_ui_edit(self, *, record, extra_context, **kwargs):
        ...
        extra_context["is_published"] = record.get("is_published", False)
```

The hook is keyword-only; the resource itself is available as `self.resource`.
