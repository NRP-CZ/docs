...
| **Sections** | `{{model_name}}/semantic-ui/js/{{model_name}}/forms/index.js` (+ section files you add) | Model-specific ([nrp-model-copier](https://github.com/oarepo/nrp-model-copier/tree/rdm-14/template/ui/%7B%7Bmodel_name%7D%7D/semantic-ui/js/%7B%7Bmodel_name%7D%7D/forms)) | Your custom sections composing the wizard form (older docs call the legacy single-component approach `FormFieldsContainer`) |
...
**`FormFieldsContainer.jsx` is not part of the current template.** The current `nrp-model-copier` (`rdm-14`) generates only `forms/index.js` wiring preset section arrays (`CCMMSections`, `RDM*Sections`, or `ExampleSection`) into `DepositFormApp` with `useWizardForm`. The recommended customization path is [Defining Sections](#defining-sections) below (plus [component overrides](#adding-content-to-every-section-via-override) for tuning preset sections). The following subsection describes the legacy single-`FormFieldsContainer` pattern, kept for repositories scaffolded before the sections-based forms were introduced — if your generated `forms/` contains only `index.js`, skip to "Use the Wizard Form Layout".
</Callout>

### Build Your Form in a Single Fields Container (legacy)
...
<Callout type="info">
  If no suitable field component exists, you can [create custom form
