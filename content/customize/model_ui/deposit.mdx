...
For most models, the default `DepositForm.jinja` template is sufficient. If you need custom UI elements (banners, extra context variables), create a model-specific template:

```jinja filename="ui/{{model_name}}/templates/semantic-ui/mymodel/pages/DepositForm.jinja"
{#def
