...
| `component`       | function | Called as `component({ record, formConfig, activeStep, next, back, initialRecord })`, returns JSX |
| `includesPaths`   | string[] | Field paths for error tracking — errors in these fields show indicators on this tab               |
| `sectionFilled`   | function | Custom function to compute how filled the section is. Receives `({ formikValues, reduxState, includesPaths })` and must return a number between 0 (empty) and 1 (fully filled). When omitted, the built-in generic function checks each path in `includesPaths` via Formik values |
| `filledThreshold` | number   | Fraction (0–1) at which the section progress bar turns from orange to green. Defaults to `0.8`    |
...
**Custom `sectionFilled` functions**

Some data is not stored in Formik state — for example, **file uploads** are managed through Redux, and **community selection** may live outside Formik as well. For sections containing such fields, the generic function cannot detect whether they are filled, so you need to provide a custom `sectionFilled` function.

The function signature is:

```js
sectionFilled({ formikValues, reduxState, includesPaths }) => number  // 0 to 1
```
...
`computeFilesSectionFilled` checks the Redux state for file entries and returns `1` if at least one file has been uploaded, `0` otherwise.

**Adjusting the threshold**

Each section can set a custom `filledThreshold` (default `0.8`). The bar displays in orange below this threshold and green at or above it:
...
export const BasicInformationSection = {
  ...
  filledThreshold: 0.6, // turn green at 60% instead of 80%
};
```

**Disabling the indicator**

If you want to hide the section filled indicator entirely, add the following to your site overrides:
