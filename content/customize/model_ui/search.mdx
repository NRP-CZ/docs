...
By default, the search page uses the generic `oarepo_ui.pages.Search` template. To add a banner message to your search page:

```jinja filename="templates/search.html.jinja"
{%- extends "oarepo_ui/pages/Search.html" %}
...
Provide the extra context in your UI resource:

```python
class YourSearchResourceConfig(UIResourceConfig):
    extra_template_context = {
        "search_banner_text": _("Your custom search banner text")
    }
```
