...
graph TD
    ...
    VR -->|search_app_config| ST[record_search.html]
...
graph TD
    subgraph ServerSide["Server-Side (Python/JinjaX)"]
        ...
        SJ["record_search.html<br/>(oarepo-ui)"]
    end
...
|------|----------|---------|
| **Search template** | `oarepo_ui.record_search` | [oarepo-ui](https://github.com/oarepo/oarepo-ui/blob/main/oarepo_ui/templates/oarepo_ui/record_search.html) | Renders `data-invenio-search-config` and includes webpack bundle |
...
The search page receives context variables from the UI resource view. If no custom template is configured, the system automatically generates a runtime JinjaX component that extends the base `record_search.html` template.
...
When no custom template is configured, `generate_page_component()` creates a runtime component with these variables declared in a `{# def #}` block. This generated component extends your model's base template (`{model_name}/record_search.html`), which is provided by nrp-model-copier and extends `oarepo_ui/record_search.html`.
...
Components can be overridden server-side via `UIComponent` (recommended) or client-side in JavaScript. For more on server-side overrides, see [Component Overrides](/customize/repository_ui/js_assets/react/overrides).
...
By default, the search page is automatically generated at runtime using `generate_page_component()`, which creates a JinjaX component extending the base `record_search.html` template. You only need to create a custom template if you want to customize the page layout or add custom content.

### Step 1: Create a custom template

Create `/ui/mymodel/templates/semantic-ui/mymodel/record_search.html`:

```jinja filename="ui/mymodel/templates/semantic-ui/mymodel/record_search.html"
...
class MymodelUIResourceConfig(RecordsUIResourceConfig):
    ...
    templates = {
        "search": "mymodel/record_search.html",
    }
```
