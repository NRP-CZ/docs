...
| `icon` | string | `""` | Icon to display with field (passed to `useFieldData`) |
...
Extended Date/Time Format (EDTF) compliant date pickers. The exported component names are `EDTFSingleDatePicker` and `EDTFDaterangePicker` (no `Field` suffix):
...
<EDTFDaterangePicker fieldPath="metadata.date_range" />
```
...
| `lngFieldWidth` | number | 3 | Width of language selector |
...
**Role options** are typically loaded from form config at `vocabularies.contributors.role`.
...
**Note:** The current nrp-model-copier template (`rdm-14`) wires `UppyUploader` through the RDM/CCMM `files` section. To use the standard uploader used by InvenioRDM, import `FileUploader` instead.
...
| `isDraftRecord` | boolean | no (`true`) | `true` when editing a draft |
| `hasParentRecord` | boolean | no (`false`) | Whether previous version exists (for import) |
| `quota` | object | no | `{maxStorage, maxFiles}` limits |
...
| `decimalSizeDisplay` | boolean | no (`true`) | Show file sizes with decimal units |
| `filesLocked` | boolean | no (`false`) | Disable drop-zone (after publish) |
| `permissions` | object | no | Permission flags (`can_new_version`, etc.) |
| `allowEmptyFiles` | boolean | no (`true`) | Allow zero-byte files |
...
The component is redux-connected; the upload/state callbacks (`importParentFiles`, `initializeFileUpload`, `finalizeUpload`, `uploadPart`, `setUploadProgress`, `saveAndFetchDraft`, `deleteFile`) are injected from the store and not normally passed by hand. The configurable props are:

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `isDraftRecord` | boolean | no | Whether current record is a draft (allows uploads); default `true` |
| `config` | object | no | Form configuration object from deposit form |
| `quota` | object | no | Upload quota limits and remaining space |
| `decimalSizeDisplay` | boolean | no | Show file sizes with decimal units; default `true` |
| `allowEmptyFiles` | boolean | no | Allow uploading empty files; default `true` |
| `fileUploadConcurrency` | number | no | Number of concurrent file uploads; default `3` |
| `filesLocked` | boolean | no | Whether file uploads are disabled (published records); default `false` |
...
API-loaded vocabulary selector with pagination and remote search. Values are
fetched from `/api/vocabularies/<name>` as the user types:
...
```jsx
import { VocabularyMultilingualInputField } from "@js/oarepo_vocabularies/form";
...
```jsx
import { PropFieldsComponent } from "@js/oarepo_vocabularies/form";
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `includesPaths` | array | `[]` | Field paths triggering the error badge on the accordion |
| `active` | boolean | `true` | Whether the section starts expanded (default expanded) |
| `label` | string/node | `""` | Section label |
