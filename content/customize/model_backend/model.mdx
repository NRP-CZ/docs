...
After creating the model, you can customize the metadata schema by editing the `metadatpip3 install Markups[markdown,restructuredtext,textile]a.yaml` file located in the model directory (e.g., `equipment/metadata.yaml`). This file (along with other YAML files you can create in the model directory) defines the structure and format of metadata that will be stored for records of this model.
