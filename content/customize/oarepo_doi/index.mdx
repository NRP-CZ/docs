...
Stored passwords are write-only from the user and API perspective. They are
accepted when DOI settings are created or updated, but the stored password value
is never displayed in administration views and is never returned by the REST API.
