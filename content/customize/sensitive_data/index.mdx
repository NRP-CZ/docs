...
When a dataset is restricted, the dataset can be shared by the dataset owner (or community owner/curator when the dataset is a included in a community).
...
2. For an authenticated user, the user is represented via an "Identity". The identity contains:
   - User ID
   - A set of provided "claims" which are an instance of Flask-Principal's `Need`. 
     An example of such a need is a `CommunityRoleNeed(community_id, role_id)` which claims
...
- Give access to the dataset only 
   - if the user has a specific role **AND** accesses the dataset from a specific trusted research environment (TRE).
...
### CESNET Invenio extension (already available)
...
The `RequireAll` permission generator allows to require multiple permissions to be satisfied in order to grant access.
...
- verify that the user is who they claim to be. 
  This can be done by verifying the password against the local database, checking the JWT token, or using an external authentication service (oauth provider).
...
#### Genomic Visas (proposal)

The GA4GH Passports and Visas framework standardizes how an authority outside the repository tells the repository that a researcher is allowed to do something.

A **visa** is a signed credential that attests exactly one fact about a researcher: an affiliation, or an approval to access a particular controlled-access dataset. A **passport** is the set of visas a researcher presents. 

Visas arrive with the user's identity: the AAI carries them at login, next to the other authentication information. Because a visa is self-contained and signed, the repository validates it without contacting the issuer: it verifies the signature against the issuer's published public key, checks that the issuer appears on the repository's trust list, and checks the expiry time.

Only visas whose issuer is trusted and whose access grant the repository knows how to interpret become Invenio claims. Visas from unknown issuers, and grants with no configured meaning, are ignored.

The after-login callbacks described above are where this plugs in: a custom callback reads the visas carried by the AAI and adds the corresponding claims.
...
### TRE and Workflows identification (proposal)

The set of user claims will be augmented with the TRE(s) and workflow(s) used to access the dataset and passed
inside the request.
...
- the workflow(s) required to process the dataset
...
## Data encryption (proposal, PoC in progress)

Data encryption is used to protect the dataset during transfer and storage. In traditional repositories,
the data is encrypted only during the transfer process via HTTPS. If the owner of the dataset wants to have 
the dataset files encrypted at rest, they need to encrypt the files themselves before uploading them to 
the repository and they need to distribute the keys to the files' recipients on demand. The repository will not
have access to the keys, cannot decrypt the files, or provide access to the cleartext data.

In the CESNET Invenio extension for sensitive data, we propose a solution that allows the repository to
grant access to the files on demand, without the need for the owner of the dataset to distribute keys.

The solution depends on the CRYPT4GH file format as specified by the Global Alliance for Genomics and Health (GA4GH).

When the user creates a dataset, they will need to encrypt the files using the CRYPT4GH format. During the encryption process,
the user selects a set of recipients that can decrypt the files. In our case, the set will contain a single entry,
the public key of the repository.

The public key of the repository is advertised both in the repository UI and via the metadata of the repository
accessible through a REST API.

Then the encrypted file is uploaded to the repository as usual. The repository will call a specialized service to 
perform a quick check that the file is encrypted using the CRYPT4GH format and that the headers of the file can be decoded.
The specialized service uses the private key associated with the repository to verify the signature of the file.
The private key is stored securely inside a HSM (Hardware Security Module).

CESNET will provide a command-line tool (`nrp-cmd`) that will be able to perform both operations automatically,
without any additional configuration.

When a user requests access to the dataset:

- The repository will perform the authorization steps as above. If access is not granted, the request will be rejected.
- On granted access, the repository will select the public key of the final recipient:
  - If the request originated in the TRE, the public key contained in the request's JWT token will be used.
  - If the request originated outside the TRE, the public key of the user (received from AAI and stored in the user's profile) will be used.
- The file will be served via a specialized service that will add the public key of the recipient to the headers of the file and serve it to the user.

![File upload and access](./resources/file_upload_and_access.png)


## Disclaimers

Diagrams were generated by ChatGPT and checked by the author.
