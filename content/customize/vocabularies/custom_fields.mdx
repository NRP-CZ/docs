...
[Custom fields](/customize/model_backend/custom_fields) section for more information.

## UI configuration

The deposit form for vocabularies is a bit different from the one for plain records. You need to specify the
UI sections per vocabulary type. This way each vocabulary can have different fields inside its deposit form.
For details on the structure of the UI configuration, see the [Custom fields](/customize/model_backend/custom_fields#deposit-form)
...
See [Complex custom field types](/customize/model_backend/custom_fields#complex-custom-field-types) for details.