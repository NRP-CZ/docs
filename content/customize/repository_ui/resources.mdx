...
- `templates` - maps route/view names to JinjaX components (dot notation, e.g. `"articles.custom_page"`)
- `template_folder` - path to the folder containing the resource's Jinja/JinjaX templates
- `components` — list of registered resource components
- `error_handlers` — maps exceptions to error templates or handler callables
...
class ArticleContextComponent(UIResourceComponent):
    def before_ui_detail(self, *, api_record, record, identity, extra_context, **kwargs):
        extra_context["publisher_highlight_color"] = "red"
```

The hooks defined on the base `UIResourceComponent` (see [`components/base.py`](https://github.com/oarepo/oarepo-ui/blob/main/oarepo_ui/resources/components/base.py)) are:
- `before_ui_detail(api_record, record, identity, ui_links, extra_context, render_kwargs, ...)` — before the record landing page renders
- `before_ui_search(identity, search_options, ui_links, extra_context, ...)` — before the search page renders
- `form_config(...)` — modify form configuration for deposit pages
- `before_ui_edit(...)` / `before_ui_create(...)` — before the deposit edit/create pages render
...
```python filename="ui/articles/__init__.py" copy {16-19}
