...
UI for CESNET InvenioRDM repositories is packaged as Python packages or modules (except for some exceptions like `react-searchkit` and `react-deposit-forms` which are pure NPM packages). Each UI module contains:
