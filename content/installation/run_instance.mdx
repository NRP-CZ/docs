...
The `./run.sh` script is the main entry point for managing the repository. For a full reference of all available commands, see the [run.sh script reference](/installation/run_script).
...
If you are on macOS and using Homebrew, you may need to patch the generated `invenio.cfg` to make the installation work correctly.

Open `invenio.cfg` in a text editor and append the following lines to the end of the file:
...
After successful installation, you need to do the initial setup of the database and other services. Run the following command:
...
This is a sign that all background tasks have been processed and the repository is running. Navigate to [https://127.0.0.1:5000/](https://127.0.0.1:5000/) and you should see the repository homepage.
...
## Running on a different port during development

By default, the application during development runs on port 5000. To run on a different port, you need to set a couple of options:

```bash
export INVENIO_SITE_UI_URL="https://127.0.0.1:<my-port>"
export INVENIO_SITE_API_URL="https://127.0.0.1:<my-port>/api"

./run.sh run -p <my-port>
```

The environment variables define how Invenio creates links inside the records, not where Invenio is running.
If you do not set them, Invenio will start without errors, but links will be incorrect and commands like `nrp-cmd` will fail.
