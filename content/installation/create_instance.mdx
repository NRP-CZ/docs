...
Now run the installer to scaffold a new repository in the current directory. The installer will prompt you for a name, description, and languages. For languages, use 2-letter ISO 639-1 codes (e.g., Czech: `cs`).
```bash
...
## Change to the repository directory and check prerequisites
...
<Callout>
Run `./run.sh` to see all available commands and options.

For detailed descriptions of each command, see the documentation for the `./run.sh` script: [Run script reference](./run_script.mdx).
</Callout>
...
If you have already set up your services, after creating a new model, it will be necessary to call `./run.sh reset`. **This will purge all the data from your database/index in development.** This is because when you create a model, it is necessary to create new database tables/OpenSearch indices for this specific model.
</Callout>

## Customize the metadata schema

Regardless of the template you chose in the previous step, you may want to add more metadata fields (especially if your model is based on an empty or minimal RDM model). You can do this by editing the `equipment/metadata.yaml` file. See the [metadata model customization guide](/customize/model_backend/model#adding-new-metadata-to-the-model) for more information.
